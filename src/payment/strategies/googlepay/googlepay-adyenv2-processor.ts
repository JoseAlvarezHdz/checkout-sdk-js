...
export default class GooglePayAdyenV2Processor {
    private _adyenClient?: AdyenClient;
    private _scriptLoader: AdyenV2ScriptLoader;

    constructor(private _store: CheckoutStore, private _paymentActionCreator: PaymentActionCreator) {
        const scriptLoader = getScriptLoader();
        this._scriptLoader = new AdyenV2ScriptLoader(scriptLoader, getStylesheetLoader());
    }

    initialize(options: PaymentInitializeOptions) {
    ...
    async _processAdditionalAction(error: unknown): Promise<InternalCheckoutSelectors> {
